<x-content :title="$title">
          ...
          <tr class="bg-white hover:bg-gray-50">
            <td class="px-2 py-2 border border-gray-300 text-center text-nowrap">
              <x-dropdown no-x-anchor class="btn-sm">
                <x-menu-item title="Edit"  wire:click.debounce.500ms="editProductContentDisplay('{{ $item['id'] }}')" role="menuitem" />
                <x-menu-item title="Delete" wire:confirm="Do you want to delete this data?"  wire:click="deleteProductContentDisplay('{{ $item['id'] }}')" role="menuitem" />
              </x-dropdown>
            </td>
  ...
  <x-modal wire:model="myModalProductContentDisplayForm" 
      ...
      {{-- <x-slot:actions>
          <x-button label="Cancel" @click="$wire.myModalProductContentDisplayForm = false" />
